"""
Financial Agent - Complete Integrated Application
Combines backend API and frontend in a single application
"""
...
app = FastAPI(
    ...
    description="Autonomous AI Financial Coach for Gig Workers",
    version="1.0.0"
)
...
async def serve_frontend():
    ...
    try:
        ...
        <html>
            <head><title>Financial Agent</title></head>
            <body>
                <h1>Financial Agent - Backend Running</h1>
...
async def health_check():
    ...
    return {
        "status": "healthy",
        "service": "financial-agent-integrated",
        "version": "1.0.0",
...
@app.get("/api/transactions/{user_id}/balance")
...
@app.get("/api/predictions/{user_id}/latest")
...
# Nudge endpoints
@app.post("/api/nudges/generate", status_code=201)
...
async def get_dashboard_data(user_id: str):
    ...
    user_transactions = [t for t in transactions_db.values() if t["user_id"] == user_id]
    recent_transactions = sorted(user_transactions, key=lambda x: x["transaction_date"], reverse=True)[:10]
    ...
    return {
        ...
        "savings": 15000,  # Mock savings amount
        "timestamp": datetime.utcnow().isoformat()
    }

if __name__ == "__main__":
    import uvicorn
    print("🚀 Starting Financial Agent - Integrated Application...")
    print("📱 Frontend: http://localhost:8000")
    print("🔧 API Documentation: http://localhost:8000/docs")
    print("🏥 Health Check: http://localhost:8000/health")
    print("🎮 Demo Setup: POST http://localhost:8000/api/demo/setup")
